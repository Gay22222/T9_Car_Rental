...
public class OwnerNotificationsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        mView = inflater.inflate(R.layout.owner_fragment_notification, container, false);

        recyclerView = mView.findViewById(R.id.recyclerView_notifications); // sửa lại ID phù hợp nếu cần
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));

        notiList = new ArrayList<>();
        notiList.add("Khách hàng đã gửi yêu cầu thuê xe.");
        notiList.add("Đơn hàng mới đang chờ xác nhận.");
        notiList.add("Khách đã thanh toán thành công.");
        notiList.add("Xe đã được trả lại.");
        notiList.add("Một đơn hàng đã bị huỷ.");
